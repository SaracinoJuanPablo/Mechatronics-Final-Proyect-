...
engine = pyttsx3.init()
#engine.setProperty('voice', 'spanish')  # Configurar idioma español
...
def generar_audio(label):
    """Genera archivo de audio para una etiqueta si no existe"""
    ...
    try:
        ...
        engine.save_to_file(label, ruta_audio)
        engine.runAndWait()
        print(f"Audio generado: {nombre_archivo}")
    except Exception as e:
        print(f"Error generando audio para {label}: {str(e)}")
