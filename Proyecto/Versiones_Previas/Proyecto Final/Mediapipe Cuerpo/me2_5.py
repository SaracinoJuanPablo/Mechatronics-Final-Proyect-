...
class SignLanguageSystem:
    ...
    def extract_landmarks(self, pose_results, hands_results):
        """Extrae y normaliza los landmarks de pose y manos, manejando casos donde faltan datos"""
        landmarks = []
        
        # Extraer landmarks de pose si están disponibles
    ...
    def collect_data(self, sign_name):
            """Recolecta datos para una seña específica automáticamente después de presionar espacio"""
            ...
            capture_delay = 0.1  # Tiempo entre capturas (0.1 segundos)
            
            print("\nInstrucciones:")
            print("1. Posiciónate frente a la cámara")
            print("2. Presiona ESPACIO para comenzar la captura automática")
            print("3. Presiona ESC para cancelar en cualquier momento")
            ...
            while True:
                ...
                if not is_capturing:
                    cv2.putText(frame, "Presione ESPACIO para iniciar la captura", 
                            (10, frame.shape[0] - 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                else:
                    cv2.putText(frame, f"Capturando... {count}/{total_samples}", 
                            (10, frame.shape[0] - 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                    
                    # Realizar capturas automáticas sin verificación de calidad
                    if current_time - last_capture_time >= capture_delay:
                        landmarks = self.extract_landmarks(pose_results, hands_results)
                        sample_path = os.path.join(sign_dir, f"sample_{count}.npy")
                        np.save(sample_path, landmarks)
                        print(f"Muestra {count + 1}/{total_samples} guardada")
                        count += 1
                ...
                if key == 32:  # ESPACIO - Iniciar/Detener captura
                    is_capturing = True
            ...
            print(f"\nTotal de muestras guardadas: {count}")
