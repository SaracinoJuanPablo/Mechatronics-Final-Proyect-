...
def speak_async(engine, text):
    """Función para ejecutar pyttsx3 en un hilo separado"""
    def speak():
        engine.say(text)
        engine.runAndWait()
...
class SignLanguageSystem:
    ...
    def evaluate(self):
        ...
        #cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))
        cap.set(cv2.CAP_PROP_FPS, 60)  # Ajusta a 30 FPS
        #cv2.namedWindow("Camera", cv2.WINDOW_NORMAL)
        #cv2.resizeWindow("Camera", 640, 480)
        ...
        # Buffer para almacenar la secuencia actual
        current_sequence = []
        predictions_buffer = []
        ...
        while True:
            ...
            # Extraer landmarks y agregar a la secuencia
            landmarks = self.extract_landmarks(pose_results, hands_results)
            current_sequence.append(landmarks)
            ...
            if len(current_sequence) == self.sequence_length:
                ...
                # Mostrar predicción si es estable
                if stability > 0.6:
