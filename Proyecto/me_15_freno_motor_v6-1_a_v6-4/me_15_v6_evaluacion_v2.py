...
###########################
# PARÁMETROS DE FILTRO DE MANO POR DISTANCIA
# Ajusta estos valores según tu configuración y pruebas
MIN_HAND_SIZE_THRESHOLD = 80   # Tamaño mínimo en píxeles (ajusta según tu cámara)
MAX_HAND_SIZE_THRESHOLD = 400  # Tamaño máximo en píxeles (opcional, pon None para desactivar)
###########################
...
def extract_hand_landmarks(frame, send_sock=None):
    ...
    # Filtro: solo tomar la mano "válida" según el tamaño
    selected_hand = None
    ...
    if results.multi_hand_landmarks:
        # Calcular tamaño de cada mano detectada
        for hand_idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
            ...
            # Calcular distancia máxima entre puntos (como "tamaño" de la mano)
            max_dist = 0
            for i in range(len(points)):
                for j in range(i+1, len(points)):
                    ...
                    if dist > max_dist:
                        max_dist = dist
            hand_sizes.append((hand_idx, max_dist))

        # Filtrar manos por tamaño
        valid_hands = [(idx, size) for idx, size in hand_sizes if size >= MIN_HAND_SIZE_THRESHOLD and (MAX_HAND_SIZE_THRESHOLD is None or size <= MAX_HAND_SIZE_THRESHOLD)]
        if valid_hands:
            # Elegir la mano más "mediana" (la más cercana al centro del rango)
            ideal_size = (MIN_HAND_SIZE_THRESHOLD + (MAX_HAND_SIZE_THRESHOLD or MIN_HAND_SIZE_THRESHOLD+200)) / 2
            selected_hand_idx, selected_size = min(valid_hands, key=lambda x: abs(x[1] - ideal_size))
            selected_hand = results.multi_hand_landmarks[selected_hand_idx]
            hands_detected = True

    if selected_hand is not None:
        # Dibujar solo la mano seleccionada
        ...
        if results.multi_handedness:
            handedness = results.multi_handedness[selected_hand_idx]
            if handedness.classification[0].label == 'Left' and not x_normalized:
                wrist = selected_hand.landmark[mp_hands.HandLandmark.WRIST]
    ...
    # Rellenar con ceros si no hay manos válidas
    ...
    if hands_detected:
        ...
        if current_time - last_hands_detected_time > 1:
